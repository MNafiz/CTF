...
list_file = os.listdir("keys_and_messages")
list_file_pem = list(filter(lambda x : ".pem" in x, list_file))

list_n = []

for pem_file in list_file_pem:
    with open(f'keys_and_messages/{pem_file}') as f:
...
for i in range(50):
    n_temp = list_n[i]
    fpb = GCD(n_21,n_temp)
    if (fpb > 1) and (n_21 != n_temp):
        print('Dapet')
        p = fpb
        break

q = n_21 // p
d = pow(e_21,-1,(p-1)*(q-1))
m = pow(c,d,n_21)

key = RSA.construct((n_21,e_21,d))
