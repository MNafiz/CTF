

# This file was *autogenerated* from the file solve.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_8 = Integer(8); _sage_const_128 = Integer(128); _sage_const_0 = Integer(0); _sage_const_7 = Integer(7); _sage_const_3 = Integer(3); _sage_const_4 = Integer(4); _sage_const_6 = Integer(6); _sage_const_32 = Integer(32); _sage_const_9 = Integer(9); _sage_const_16 = Integer(16); _sage_const_256 = Integer(256)
from sage.all import *
from aes import AES
import os
from pwn import *

NC = "nc 103.145.226.206 13831".split()

r = remote(NC[_sage_const_1 ], NC[_sage_const_2 ])


do_nothing = lambda *x: None

def bytes2mat(b):
    a = []
    for i in b:
        tmp = bin(i)[_sage_const_2 :].zfill(_sage_const_8 )
        for j in tmp:
            a.append(int(j))
    return Matrix(GF(_sage_const_2 ), a)

def mat2bytes(m):
    a = ""
    for i in range(_sage_const_128 ):
        a += str(m[_sage_const_0 , i])
    a = [a[i:i+_sage_const_8 ] for i in range(_sage_const_0 , _sage_const_128 , _sage_const_8 )]
    a = [int(i, _sage_const_2 ) for i in a]
    return bytes(a)

I = identity_matrix(GF(_sage_const_2 ), _sage_const_8 )
X = Matrix(GF(_sage_const_2 ), _sage_const_8 , _sage_const_8 )
for i in range(_sage_const_7 ):
    X[i, i+_sage_const_1 ] = _sage_const_1 
X[_sage_const_3 , _sage_const_0 ] = _sage_const_1 
X[_sage_const_4 , _sage_const_0 ] = _sage_const_1 
X[_sage_const_6 , _sage_const_0 ] = _sage_const_1 
X[_sage_const_7 , _sage_const_0 ] = _sage_const_1 

C = block_matrix([
    [X, X+I, I, I],
    [I, X, X+I, I],
    [I, I, X, X+I],
    [X+I, I, I, X]
])

zeros = Matrix(GF(_sage_const_2 ), _sage_const_8 , _sage_const_8 )
zeros2 = Matrix(GF(_sage_const_2 ), _sage_const_32 , _sage_const_32 )
o0 = block_matrix([
    [I, zeros, zeros, zeros],
    [zeros, zeros, zeros, zeros],
    [zeros, zeros, zeros, zeros],
    [zeros, zeros, zeros, zeros]
])

o1 = block_matrix([
    [zeros, zeros, zeros, zeros],
    [zeros, I, zeros, zeros],
    [zeros, zeros, zeros, zeros],
    [zeros, zeros, zeros, zeros]
])

o2 = block_matrix([
    [zeros, zeros, zeros, zeros],
    [zeros, zeros, zeros, zeros],
    [zeros, zeros, I, zeros],
    [zeros, zeros, zeros, zeros]
])

o3 = block_matrix([
    [zeros, zeros, zeros, zeros],
    [zeros, zeros, zeros, zeros],
    [zeros, zeros, zeros, zeros],
    [zeros, zeros, zeros, I]
])

S = block_matrix([
    [o0, o1, o2, o3],
    [o3, o0, o1, o2],
    [o2, o3, o0, o1],
    [o1, o2, o3, o0]
])

M = block_matrix([
    [C, zeros2, zeros2, zeros2],
    [zeros2, C, zeros2, zeros2],
    [zeros2, zeros2, C, zeros2],
    [zeros2, zeros2, zeros2, C]
])

R = M*S
A = S*(R**_sage_const_9 ) # sorry for the inconsistency in the variable name, this is supposed to be SA^9 that I talked about

c = AES(os.urandom(_sage_const_16 ))
c._sub_bytes = do_nothing
p = b"Very secret text"
ct = c.encrypt(p)
p2 = b"Known plaintextt"
ct2 = c.encrypt(p2)

p2 = bytes2mat(p2).transpose()
ct2 = bytes2mat(ct2).transpose()

K = ct2 - A*p2
recovered_plaintext = mat2bytes((A.inverse() * (bytes2mat(ct).transpose() - K)).transpose())
print(recovered_plaintext)


r.recvuntil(b"gift: ")
nonce_flag , ctflag = eval(r.recvline(_sage_const_0 ))
nonce_flag = bytes.fromhex(nonce_flag)
nonce_flag_1 = nonce_flag + int.to_bytes(int(_sage_const_1 ), int(_sage_const_4 ), 'big')
nonce_flag_2 = nonce_flag + int.to_bytes(int(_sage_const_2 ), int(_sage_const_4 ), 'big')
ctflag = bytes.fromhex(ctflag)

payload = b"00"*_sage_const_32 

r.sendlineafter(b"(hex): ", payload)
nonce, enc = eval(r.recvline(_sage_const_0 ))
nonce = bytes.fromhex(nonce)
enc = bytes.fromhex(enc)
nonce_1 = nonce + int.to_bytes(int(_sage_const_1 ), int(_sage_const_4 ), 'big')
nonce_2 = nonce + int.to_bytes(int(_sage_const_2 ), int(_sage_const_4 ), 'big')
enc_1 =  enc[:_sage_const_16 ]
enc_2 = enc[_sage_const_16 :]

print(xor(enc_1, enc_2))


payload = b"00"*_sage_const_32 

r.sendlineafter(b"(hex): ", payload)
nonce, enc = eval(r.recvline(_sage_const_0 ))
nonce = bytes.fromhex(nonce)
enc = bytes.fromhex(enc)
nonce_1 = nonce + int.to_bytes(int(_sage_const_1 ), int(_sage_const_4 ), 'big')
nonce_2 = nonce + int.to_bytes(int(_sage_const_2 ), int(_sage_const_4 ), 'big')
enc_1 =  enc[:_sage_const_16 ]
enc_2 = enc[_sage_const_16 :]

print(xor(enc_1, enc_2))

enc_1_xor_enc_2 = xor(enc_1, enc_2)

hh = xor(ctflag[:_sage_const_16 ], ctflag[_sage_const_16 :], enc_1_xor_enc_2)

print(hh)


print(mat2bytes(p2.transpose()))



a = [dict() for i in range(_sage_const_16 )]

mapping = []

print(len(nonce_flag))
tempel = int.to_bytes(int(_sage_const_1 ), int(_sage_const_4 ), 'big')
payload = b"00"*_sage_const_32 
Ainv = A.inverse()

for i in range(_sage_const_16 ):
    print(i+_sage_const_1 )
    j = _sage_const_0 
    while len(a[i]) != _sage_const_256 :
        j += _sage_const_1 
        print(j)
        r.sendlineafter(b"(hex): ", payload)
        nonce, enc = eval(r.recvline(_sage_const_0 ))
        nonce = bytes.fromhex(nonce) + tempel
        if nonce[i] != nonce_flag_1[i]:
            continue
        enc = bytes.fromhex(enc)[:_sage_const_16 ]

        enc = bytes2mat(enc).transpose()

        hoho = Ainv * enc

        hehe = mat2bytes(hoho.transpose())
        mapping.append(hehe[i])
        print(bytes(mapping), i)
        break


mapping_mat = bytes2mat(bytes(mapping)).transpose()
enc = A * mapping_mat
enc_nonce = mat2bytes(enc.transpose())


print(xor(ctflag,enc_nonce))
    

# NCW23{yeah_ill_just_use_this_flag_1234}


r.interactive()

